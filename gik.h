...
void matrisToMat(matris *mat)
{
	...
	matris *satirPtr = mat; matris *sutunPtr = mat;
...
matris* matrisBuyut(matris *anaMat)
{
	...
	buyukMatYatay = buyukMat; buyukMatDikey = buyukMat;
...
void asindirmaElemanEkle(int sayi,matris **ptr,matris **gecici,matris **satirSonu)
{
	...
	if (sayi == 255)
	{
		if ((*ptr) == (*satirSonu))
		{
			if ((*ptr)->yukari != NULL)(*ptr)->yukari->deger = 255;
			if ((*ptr)->assagi != NULL)(*ptr)->assagi->deger = 255;
			if ((*ptr)->sonraki != NULL)(*ptr)->sonraki->deger = 255;
			if ((*ptr)->onceki != NULL)(*ptr)->onceki->deger = 255;
			(*gecici) = (*gecici)->assagi;
			(*ptr) = (*gecici);
			(*satirSonu) = (*satirSonu)->assagi;
		}
...
matris * konvolusyon(int sayi,matris *ana,matris *maske)
{
	...
	for (int i = 0; i <= assagiGit; i++)
	{
		...
		{
			if (sayi == 0) { bulaniklastirmaIslemi(ptrSatir, ikinciPtr, maske, &ptr, &gecici, &satirSonu);}
			else if (sayi == 1) { laplasIslemi(ptrSatir, ikinciPtr, maske, &ptr, &gecici, &satirSonu);}
			else if (sayi == 2) { ortancaIslemi(ptrSatir, ikinciPtr, maske, &ptr, &gecici, &satirSonu);}
			else if (sayi == 3) { genisletmeIslemi(ptrSatir, ikinciPtr, maske, &ptr, &gecici, &satirSonu);}
			else if (sayi == 4) { asindirmaIslemi(ptrSatir, ikinciPtr, maske, &ptr, &gecici, &satirSonu);}
...
int esikDegeri(matris *ana)
{
	...
	for (int j = 0; j < histogramSize; j++) { toplam2 += j * histogram[j]; }

	for (int j = 0; j < histogramSize; j++)
	{
		ag1 = 0;		
		for (int k = 0; k <= j; k++){ag1 += histogram[k];}
		ag2 = (agtop - ag1); 
...
matris* keskinlestirmeIslemi(matris *bulanik, matris *mat)
{
	...
	satirmat = mat; sutunmat = mat;
	//fark =SiyahBeyaz(fark);
	satirFark = fark; sutunFark = fark;
	//MatrisGoster(fark);
...
int nesneBul(matris *olasiNesne,int nesneSayisi)
{
	...
	while (stckLengt(&stck)!=0)
	{
		...
		if (ara->assagi != NULL) { if (ara->assagi->deger == 0) {eklensin(&stck,ara->assagi); } }
		if (ara->yukari != NULL) { if (ara->yukari->deger == 0) { eklensin(&stck, ara->yukari); } }
		if (ara->sonraki!= NULL) { if (ara->sonraki->deger == 0) {eklensin(&stck, ara->sonraki); } }
		if (ara->onceki != NULL) { if (ara->onceki->deger == 0) { eklensin(&stck, ara->onceki); } }
...
matris *siyahBeyazNesneBul(matris *mat, matris *maske)
{
	...
	for (int i = 0; i < yanKay; i++) {	matPtr = matPtr->sonraki;}
	for (int i = 0; i < assagiKay; i++) { matPtr = matPtr->assagi; }
	matSutun = matPtr;
